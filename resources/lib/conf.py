class configuration:
	...
	def read(self, config_file):
		...
		try:
			...
			self.log 						= config.get(section, 'log')
		except configparser.Error as err:
			print('There is an error in pyhame.conf (%s)' % err)
			sys.exit(1)
		## Remote ##
	...
	def check(conf):
		...
		if not os.path.exists(conf.template_path) or not os.path.exists("%s/index.html" % conf.template_path) or not os.path.exists("%s/view.html" % conf.template_path):
			...
			sys.exit(0)
		# Check if log is set
		if not conf.log and conf.log != "false" and conf.log != "true":
			print(" \033[93m::\033[0m \"log\" must be true or false in pyhame.conf (other section)")
			sys.exit(0)
		## Remote section
