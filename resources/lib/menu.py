					...
					if oDir == content_folder:
						...
						for f in special_files:
							if i == f:
								...
								break
						if not tmp_check:
							if check.file_extension(i, extensions_to_render):
						...
						if check.file_extension(i, extensions_to_render):
							...
							file_info = ("%s/%s.html" % (remove.content.folder_name(quote(oPaths)), quote(i)), remove.extension(i, extensions_to_render))
							sub_file_list.append(file_info)
			break
		if oDir != content_folder:
			if os.listdir(oDir):
